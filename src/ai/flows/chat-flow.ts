...
export type ChatOutput = {
  content: string;
  type: 'text' | 'image' | 'video_pending' | 'r34_result' | 'spotify_track' | 'error';
  media?: string;
  videoUrl?: string;
  spotifyData?: any;
};
...
4. PROTOCOLO R34 ATIVO -> Use ACTION:R34_SEARCH(slug).
5. CHAT ESTRATÉGICO -> ACTION:CHAT(resposta tática).
...
export async function chatFlow(input: ChatInput): Promise<ChatOutput> {
  ...
  const systemPrompt = input.r34Active 
    ? `${CORE_DNA}\n\n[PROTOCOLO R34 ATIVO]: Você pode usar ACTION:R34_SEARCH(slug).`
    : CORE_DNA;
  ...
  try {
    ...
    if (/ACTION:R34_SEARCH/i.test(content) && input.r34Active) {
      const match = content.match(/ACTION:R34_SEARCH\s*[:(]?\s*([^)]+)(?:\)|$)/is);
      const slug = (match && match[1]) ? match[1].trim().toLowerCase().replace(/\s+/g, '-') : input.prompt.toLowerCase().replace(/\s+/g, '-');
      return {
        content: `Sincronizando Protocolo R34: ${slug}`,
        type: 'r34_result',
        videoUrl: slug
      };
    }
